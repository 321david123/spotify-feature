...
interface SpotifyArtistWidgetProps {
...
// Default props
const defaultProps: Partial<SpotifyArtistWidgetProps> = {
  showSeeMore: true,
  initialTrackCount: 5,
  showPreviews: true,
  loadingMessage: "Loading artist...",
  errorMessage: "Could not load artist data"
}
...
export default function SpotifyArtistWidget({ 
  artistName, 
  className = "",
  apiEndpoint,
  showSeeMore = true,
  ...
  const finalLoadingMessage = loadingMessage || defaultProps.loadingMessage || "Loading artist..."
  const finalErrorMessage = errorMessage || defaultProps.errorMessage || "Could not load artist data"
  ...
  if (isLoading) {
    return (
      <div className={`w-full max-w-md p-6 bg-[#28152c] rounded-lg text-white font-sans shadow-lg ${className}`}>
        ...
        <p className="text-center text-gray-400">{finalLoadingMessage.replace('{artist}', artistName)}</p>
      </div>
    )
  }
      ...
      <div className={`w-full max-w-md p-6 bg-[#28152c] rounded-lg text-white font-sans shadow-lg ${className}`}>
        <div className="flex items-center justify-center min-h-[200px]">
          <div className="text-center">
            ...
            <p className="text-gray-400 mb-2">{finalErrorMessage}</p>
            <p className="text-sm text-gray-500">{error || 'Artist not found'}</p>
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div className={`w-full max-w-md bg-[#28152c] rounded-lg text-white font-sans shadow-lg overflow-hidden ${className}`}>
      {/* Artist Header */}
      <div className="p-6 pb-4">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="text-xl font-bold truncate" title={artistData.name}>
              <a 
        ...
        {/* Tab Navigation */}
        <div className="flex gap-1 bg-[#2d1a36] rounded-lg p-1">
          <button
            onClick={() => {
              setActiveTab('top')
      ...
      <div className="px-6 pb-6">
        <div className="space-y-2">
          {displayedTracks.map((track, index) => (
            <div 
              key={track.id} 
              className="flex items-center gap-3 p-3 bg-[#2d1a36] rounded-lg hover:bg-[#3a2342] transition-colors group"
            >
              ...
              <div className="flex items-center gap-3 flex-1 min-w-0">
                <div className="flex-shrink-0">
                  <Image
                    src={track.album.images[0]?.url || "/placeholder.svg?width=48&height=48&text=Album"}
                ...
                <div className="flex-1 min-w-0">
                  <h3 className="text-sm font-medium truncate" title={track.name}>
                    <a 
                  ...
                  <p className="text-xs text-gray-400 truncate" title={track.album.name}>
                    {track.album.name}
                  </p>
                </div>
              </div>
        ...
        {hasMoreTracks && (
          <button
            ...
            className="w-full mt-4 py-2 px-4 text-sm font-medium text-gray-300 hover:text-white bg-[#2d1a36] rounded-lg hover:bg-[#3a2342] transition-colors flex items-center justify-center gap-2"
          >
            {showAllTracks ? (
              <>
